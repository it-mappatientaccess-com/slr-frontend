...
export const fetchUsersData = () => async (dispatch) => {
  try {
    ...
    dispatch(
      UserManagementActions.setError({ error: "Failed to fetch users." })
    );
    dispatch(UserManagementActions.setProgress({ progress: 100 }));
  }
};
export const setUsersData = (username, newDetails) => async (dispatch) => {
    ...
    if (response.data.status === 200) {
      dispatch(
        UserManagementActions.setUpdateResponse({
          type: "success",
          message: `User updated successfully: ${response.data.username}`,
        })
      );
    } else {
      dispatch(
...
export const deleteUserData = (username) => async (dispatch) => {
  try {
    ...
    console.error(error);
    dispatch(
      UserManagementActions.setError({ error: "Failed to delete user." })
    );
  }
};
